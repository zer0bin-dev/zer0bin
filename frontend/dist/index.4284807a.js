...
let apiUrl = "https://stepbro.voring.me/api";
function postPaste(content, callback) {
    ...
    $.ajax({
        ...
        error: function(xhr) {
            callback(JSON.parse(xhr.responseText || `{"data": { "message": "An unkown error occured!" } }`));
        }
    });
}
function getPaste(id, callback) {
    $.ajax({
        ...
        error: function(xhr) {
            callback(JSON.parse(xhr.responseText || `{"data": { "message": "Unknown error occurred.." } }`));
        }
    });
}
function newPaste() {
...
function viewPaste(content) {
    lineNumbers.html("");
    for(let i = 1; i <= content.split("\n").length; i++)lineNumbers.append(`<div>${i}</div>`);
...
saveButton.click(function() {
    ...
    postPaste(editor.val(), function(err, res) {
        if (err) addMessage(err["data"]["message"]);
        else window.location.href = `/${res["data"]["id"]}`;
    });
});
...
//# sourceMappingURL=index.4284807a.js.map
